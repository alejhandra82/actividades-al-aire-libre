...
export class PerfilUsuarioComponent implements OnInit {
  ...
  constructor(
    ...
    this.perfilForm = this.fb.group({
      nombreUsuario: [
        ...
        [
          Validators.required,
          Validators.pattern(/^[a-zA-Z0-9ÁÉÍÓÚáéíóúñÑ ]{2,20}$/),
        ],
      ],
      apellidoUsuario: [
        '',
  ...
  cargarDatosUsuario(id: number): void {
    this.usuarioService.getUsuarioById(id).subscribe({
      next: (data) => {
        ...
        this.usuario = data;
        this.nombreFamilia =
          this.usuario.familiaDTO?.nombreFamilia || 'Sin familia';
        ...
        this.usuarioService.getFotoUsuario(data.idUsuario).subscribe({
          ...
          error: (err) => {
            console.error('Error al obtener SAS de la foto del usuario:', err);
            this.cargarFotoDefault();
          },
        });
      } else {
        this.cargarFotoDefault();
  ...
  guardarCambios(): void {
    ...
    this.usuarioService
      .actualizarUsuario(this.usuarioId, usuarioActualizado)
      .subscribe({
        next: () => {
          ...
          if (file) {
            this.usuarioService.subirFoto(this.usuarioId!, file).subscribe({
                ...
                this.usuarioService.getFotoUsuario(this.usuarioId!).subscribe({
                  next: (sasUrl) => {
                    ...
                    this.alertService.success(
                      'Éxito',
                      'Usuario actualizado correctamente con nueva foto.'
                    );
                  },
                  error: () => {
                    this.alertService.error(
                      'Error',
                      'Foto subida pero no se pudo obtener la URL.'
                    );
                  },
                });
              },
              error: () =>
                this.alertService.error(
                  'Error',
                  'Datos guardados pero la foto no se pudo subir.'
                ),
            });
          } else {
            this.alertService.success(
  ...
  private cargarFotoDefault(): void {
  this.usuarioService.obtenerFotoDefault().subscribe({
    next: (sasUrl) => this.imagenPreview = sasUrl,
    error: (err) => {
      console.error('Error al obtener SAS de la foto por defecto:', err);
      // Fallback a URL pública si falla
      this.imagenPreview =
        'https://airelibrestorage.blob.core.windows.net/imagenes/usuarios/default.png';
    },
  });
}
  ...
  //Botón crear Familia
  public irACrearFamilia(): void {
    this.router.navigate(['/perfil-familia']);
  }
}
